...
$(function(){
  ...
  BBSearch = function( opts ){
    ...
    this.search = function(){
      ...
      console.log( "BBSearch::SearchingQuery", this.inputElement.val() );
      this.eventAggregator.trigger( "bbsearch:start" );
      this.results.query = this.inputElement.val();
      this.results.fetch();
    }
  ...
  BBSearch.Backbone.InputView = Backbone.View.extend({
    ...
    keyup: function( event ){
      if( this.previousVal != this.$el.val() ) {
        ...
        this.timer = setTimeout( $.proxy( this.options.bbsearch.search, this.options.bbsearch ) , 500 );
        this.previousVal = this.$el.val();
      }
    },
  });
